...
{
    public class SyncPurchaseFormXml
    {
        public SyncPurchaseFormXml(string formVersionId, UrgentLevel Urgentlevel, string account, string userGuid, string userName, DataRow head, List<SW_GRID> grid)
        {
        ...
        public string ConvertToFormInfoXml(string appendix, string applicantId, string applicantAc, string applicantNa)
        {
            //表單表頭資訊
            ...
            foreach (var Propertie in this.Fields.GetType().GetProperties())
            {
                ...
                switch (field.FieldType)
                {
                                    ...
                                    switch (row2)
                                    {
                                        ...
                                        case "SZFEA":
                                            ...
                                            else if (row.CDEC.Trim() == "4")
                                            {
                                                ...
                                                foreignTotal += Convert.ToDouble(row.外幣總價) / 10000;
                    ...
                    default:
                        ...
                        else if (field.FieldId == "appendix")
                        {
                            if(!string.IsNullOrEmpty(appendix))
                                FieldItemXE.Add(new XAttribute("fieldValue", appendix));
                            else
                                FieldItemXE.Add(new XAttribute("fieldValue", ""));
                        }
                        else if (field.FieldId == "RApplicant")
                        {
                ...
                if (field.FieldId != "formNo" && field.FieldId != "total" && field.FieldId != "foreignTotal" && field.FieldId != "appendix" && field.FieldId != "RApplicant" && field.FieldId != "DETAIL" && head[field.FieldId].ToString() != "")
                {
                    FieldItemXE.Add(new XAttribute("fillerName", this.UserName));
                    FieldItemXE.Add(new XAttribute("fillerUserGuid", this.UserGuid));
                    FieldItemXE.Add(new XAttribute("fillerAccount", this.Account));
    ...
    public class FieldList
    {
        ...
        public Field _RApplicantField { get; set; } = new Field() { FieldType = FieldType.singleLineText, FieldId = "RApplicant" };
        ...
        public Field _appendixField { get; set; } = new Field() { FieldType = FieldType.singleLineText, FieldId = "appendix" };
